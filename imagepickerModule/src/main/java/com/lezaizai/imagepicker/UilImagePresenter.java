...
public class UilImagePresenter implements ImagePresenter{
    ...
    public View onPresentImage(Context context, String imageUri, int size) {
        ...
        ImageLoader.getInstance().displayImage(scheme.wrap(imageUri), img, new DisplayImageOptions.Builder()
    ...
    @Override
    public void onPresentWebImage(ImageView imageView, String imageUri, int size) {
        ImageDownloader.Scheme scheme = ImageDownloader.Scheme.FILE;

        ImageLoader.getInstance().displayImage(imageUri, imageView, new DisplayImageOptions.Builder()
                .showImageOnLoading(R.drawable.default_img)
                .cacheInMemory(true)
                .cacheOnDisk(true)
                .considerExifParams(true)
